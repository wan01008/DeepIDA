...
def IDA_solver(data,group_label,Gamma_old,rho):
    ...
    for k in range(num_of_views):
        ...
        #Added the small perturbation to increase stability 
        D1 = D1 + 0.000001 
    ...
    for d in range(num_of_views):
        ...
        #add the small perturbation
        need = need + 0.000001*torch.eye(need.shape[0])
...
def variable_select_bs(data_all,Y,bt_seed,model_seed,structure,n_epoch=20):
    ...
    # Bootstrap the sample index to get sample bootstrap index and oob index 
    n_select,n_oob = index_sampler(list(range(0,n_sample)),1,bt_seed)

    # Bootstrap the varaible index of each view
    p_number = []
    p_select = []
